# 高迪书法字库预处理工具

> 数字时代，风格从字库开始。

## 开发初衷

这是一个为书法爱好者制作手写字体的软件。2018 年，我跟风一款手机 App，耗时 2 个月完成了 GB2312 书法字库。后来接触专业字库软件，发现手机 APP 制作的字库精度很低，对书法字体损失很大，也难以继续迭代改进。网上的 Python 改进方案操作繁琐，不符合书法爱好者的使用习惯。本程序经历了小半年的迭代开发，实践和错误更新，现在开源发布。

## 书法爱好者做字库的痛点

1. 手写按规范填格枯燥漫长，日常习惯是写长篇作品，更适合日积月累创作；
2. 爱好者书写质量不稳定，需自由筛选优质字迹；
3. 手写长篇作品可作为字库素材，但预处理、人工编辑流程极繁琐。

## 高迪行书字库开发困境

实际制作高迪行书字库时，发现新难题：

1. 古典书法素材以繁体字为主，有效字符少（如《集王圣教序》仅 700 多不重复字），而港台一级繁体库（4800 字）和 GB2312（6700 字），多数字仍需手写。需要软件输出所需文本。
2. 手写近万字后，覆盖 GB2312、港台一级繁体库，满足日常 99.9% 需求，但仍有零星生僻字缺失，影响最终效果。

## AI 字体工具的诞生与局限

为补全生僻字，基于开源项目 zi2zi 开发**[高迪书法字库 AI 字体工具](https://github.com/gaudi1209/ai-font-tool)**：

1. 功能：导入缺失字符文本、训练万字基础模型、生成生僻字、OCR 校验筛选；
2. 局限：当前 AI 技术不成熟，生成错误率高，多重迭代后仅 60% 勉强可用；
3. 最终方案：手动补写 4000 多字，覆盖 GBK 字符集，这类字使用频率低，不影响整体书法风格。

两个工具的对接方式：本工具导出 BMP + CSV + 源文件映射（见步骤 5.1），AI 字体工具按 CSV 读 label、按映射对齐样本图。本工具的标注质量（OCR 自动 + 人工修正）直接决定 AI 训练输入质量。

## 工具核心功能

支持导入手机拍摄的高精度书法图片（约 1/6 四尺对开）：

| 步骤 | 功能 | 说明 |
|------|------|------|
| 1. 切割布局 | 自动检测 + 手动调整 | 解决书法作品行列间距不均问题 |
| 2. 切割调整 | 逐字检查 | 删除噪点、错字、空白 |
| 3. 缩放校正 | 统一像素、居中排版 | 生成标准尺寸单字图 |
| 4. 标注出图 | 批量/手动标注、繁简转换 | 支持繁简混合模式、CJK 扩展区字符 |
| 5. 导出命名 | FontLab 标准命名 | BMP 字符 `uniXXXX`，扩展区字符 `uXXXXX` |

### 步骤 1：切割布局

![切割布局](docs/images/图一.png)

- 上传手机拍摄的书法图片
- 软件自动检测行列切割线
- 可手动拖动切割线调整位置，Shift+双击添加新切割线

### 步骤 2：切割调整

![切割调整](docs/images/图二.png)

- 逐字检查切割结果
- 删除噪点、错字、空白切片

### 步骤 3：缩放校正

![缩放校正](docs/images/图三.png)

- 统一字符像素尺寸
- 自动居中排版

### 步骤 4：标注出图

![标注出图](docs/images/图四.png)

- 三种标注模式：繁简混合、以简为主、以繁为主
- 支持批量标注和手动逐字标注
- 导出后自动清理中间文件

### 步骤 5：导出

![导出结果](docs/images/图五.png)

- 输出 FontLab 标准命名的字符图片
- 重复字符自动添加后缀区分

#### 5.1 导出产物：图片 + CSV + 源文件映射（原子化）

「**导出训练包**」按钮**一次**产生三件套，全部放在**同一时间戳子目录**（`data/sessions/<hash>/exported/<时间戳>/`），保证原子化和跨产物对得上号：

| 产物 | 用途 |
|---|---|
| `*.png`（白底黑字 460 字符 + 26px padding） | FontLab / AI 字体工具训练的样本图。命名 `uniXXXX[_NN].png`，同字多个样本自动加 `_01/_02` 后缀去重 |
| `fontlab_<时间戳>.csv` | **字符图 → 汉字/码点/简繁对照表**。FontLab 按文件名识别；AI 字体工具（[高迪书法字库 AI 字体工具](https://github.com/gaudi1209/ai-font-tool)）按 CSV 读 label |
| `fontlab_<时间戳>.source_map.json` | **`scaled_NNNN.png`（源切图名）→ 导出文件名** 的映射。下游 AI 工具按源切图名加载样本图时，用这表把样本对到 CSV 某一行 |

**CSV 列结构**：
```
filename,unicode,character,simplified,traditional
uni7684_02.png,U+7684,的,的,的
```

- `filename` 唯一——同字多个样本带 `_01/_02` 后缀。**重复 sample 不重复同名**，否则 25 张「的」落到磁盘互相覆盖，AI 训练样本从 320 跌到 152
- `unicode` / `character` 用作 AI 训练的 label；`simplified` / `traditional` 是用户在 /annotate 框里填的内容（OCR 自动标注后保存到 ocr_annotations.json，重导时会自动填上）

**关键约定**：CSV 文件名必须与磁盘实际图文件名**完全一致**。本工具图片导出和 CSV 导出共享同一套 `char_counts` 计数器，**同字样本在两边都加相同后缀**，对得上号。

**两种用法**：

- **手工用 FontLab 拼字体**：把子目录里所有 PNG + CSV 一起拖进 FontLab，按文件名识别，无需 source_map。
- **AI 字体训练**：必须按 source_map 把 `scaled/` 里的源切图改名/复制成导出文件名，再让训练脚本按 CSV 读 label。

#### 5.2 清理过程图（可选）

导出后**不再自动**清理 `cutting_output/`、`scaled/`、`ocr_tasks/`（保留以便回溯错误 / 重新导出）。需要腾出磁盘时点 toolbar「清理过程图」按钮：

- 莫兰迪浅珊瑚色按钮（破坏性操作温和警示）
- 一次 confirm 弹窗：列出要删的清单（cutting_output/scaled/ocr_tasks），点取消则不执行
- 后端 `/api/cleanup_intermediate` 加 `keep_exported=True`，只清过程目录不动 `exported/`
- 标注数据 (`ocr_annotations.json`) 与 cutting.json 不受影响；OCR 已落盘的标注，下次再 OCR 会按文件名前缀 skip

## 快速开始

### 方式一：独立运行版（推荐）

从 [GitHub Release](https://github.com/gaudi1209/gaudi-font-preprocess/releases) 下载最新的 `高迪书法字库预处理工具.zip`，解压后双击 `run.bat` 即可使用，无需安装 Python 及任何依赖。

### 方式二：从源码运行

```bash
# 克隆仓库
git clone https://github.com/gaudi1209/gaudi-font-preprocess.git
cd gaudi-font-preprocess

# 推荐：使用 uv（自动创建虚拟环境并安装依赖）
uv sync

# 或：使用 pip
pip install -r requirements.txt

# 启动（推荐双击 run.bat）
python app.py
```

启动后浏览器打开 `http://localhost:7500`。

## 技术栈

- **后端**：Python + Flask
- **图像处理**：OpenCV + Pillow
- **OCR**：EasyOCR
- **繁简转换**：OpenCC

## 相关项目

- [高迪书法字库 AI 字体工具](https://github.com/gaudi1209/ai-font-tool) - 基于 zi2zi 的 AI 字体生成工具

## 许可

MIT License
