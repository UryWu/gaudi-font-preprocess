...
设计取舍：为什么用「矩形拼接」而不是平滑轮廓
--------------------------------------------
每列笔画生成一个轴对齐矩形，放大看边缘是一级级台阶。**这是刻意保留的**：
矩形是逐像素贴合原图的，手写的笔触瑕疵（飞白、墨团、毛边、枯笔）都留下来了；
描成平滑轮廓会把这种「手写味」磨掉。

若将来确实要平滑轮廓，可改用 `cv2.findContours` + `approxPolyDP`（本项目已依赖
OpenCV）。实测同一套 152 字形的对比：点数 507,992 → 13,726、字体 1.43MB → 46KB，
代价是丢掉笔触细节。
